...
本格的インストールコマンド(未安全検証): `install.sh`
...
## MDM機能の現状・制約（2025/08時点）

### 動作する主な機能
- 端末登録（enroll）・認証（ID/トークン）
- ポリシー配布（Wi-Fi/Proxy/証明書/プリンタ/アプリ制限/スクリーンタイム）
- コマンド配信（画面ロック/通知/任意コマンド実行）
- 画面の静止画取得（生徒に通知設計可）
- ファイル配布・回収（授業課題の配布/提出）
- Broadcast（クラス全員へメッセージ）
- ロール/権限による管理者・教師・ITサポートの区別
- テレメトリ（Web履歴・画像メタ等の収集、ポリシーでON/OFF可）

### 動作しない/未実装・制約事項
- 端末の「教師/生徒」区分は自動判別されず、サーバー管理者が後から端末ごとにポリシーや権限を割り当てる必要あり
- enroll時にrole情報は送信されない（今後provision.json等で拡張予定）
- サーバー・クライアント間通信は現状HTTP（本番運用時はTLS必須）
- サーバーのユーザー管理・認証はPoCレベル（初期パスワード/OTP、JWT）
- サーバーの永続DBはJSONファイルベース（大規模運用時はDB化推奨）
- サーバー・クライアントともにエラーハンドリングやセキュリティ強化は今後の課題
- 生徒端末の監視項目は最小限・明示を推奨（学校規程・保護者同意に注意）
- サーバーの管理GUIはPoCレベル（API/簡易クライアントのみ）

### 導入・運用上の注意
- サーバーURL・トークンは `/etc/sis/sis.conf` で明示的に指定が必要
- enroll後、端末ごとにサーバー管理画面でロール/ポリシー割当を行う
- 本番導入時はTLS終端・認証強化・DB化・監査ログ保全等のハードニングを推奨
